<script lang="ts">
    ...
    function syncSelection() {
      ...
      start.selectNodeContents(editorRef);
      start.setEnd(range.startContainer, range.startOffset);
      selectionStart = start.toString().length;
  
      const end = range.cloneRange();
      end.selectNodeContents(editorRef);
      end.setEnd(range.endContainer, range.endOffset);
      selectionEnd = end.toString().length;
    }
    ...
    function getNodeForCharacterOffset(
      ...
      for (let node of root.childNodes) {
        if (node.nodeType === Node.TEXT_NODE) {
          ...
          // If the node is not a text node, you might recursively check its children.
