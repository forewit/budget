  ...
  function syncSelection() {
    ...
    if (range.startContainer === editorRef) {
      if (range.startOffset > 0) {
        let remainingStartOffset = range.startOffset;
        for (let node of editorRef.childNodes) {
          const textLength = node.textContent?.length ?? 0;
          if (textLength <= 0) continue;
          start++;
          remainingStartOffset -= textLength;
          if (remainingStartOffset <= 0) break;
        }
      }
    } else {
      for (let node of editorRef.childNodes) {
    ...
    if (range.endContainer === editorRef) {
      if (range.endOffset > 0) {
        let remainingEndOffset = range.endOffset;
        for (let node of editorRef.childNodes) {
          const textLength = node.textContent?.length ?? 0;
          if (textLength <= 0) continue;
          if (remainingEndOffset > 0) end++;
          remainingEndOffset -= textLength;
          if (remainingEndOffset <= 0) break;
        }
      }
    } else {
      for (let node of editorRef.childNodes) {
